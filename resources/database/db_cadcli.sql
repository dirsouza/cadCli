...
CREATE TABLE IF NOT EXISTS `tbcompany` (
  ...
  INDEX `tbcompany_tbaddress_pk` (`idAddress` ASC),
  INDEX `tbcompany_tbcontact_pk` (`idContact` ASC),
  CONSTRAINT `tbcompany_tbaddress_fk`
    FOREIGN KEY (`idAddress`)
    REFERENCES `tbaddress` (`idAddress`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `tbcompany_tbcontact_fk`
    FOREIGN KEY (`idContact`)
    REFERENCES `tbcontact` (`idContact`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
START TRANSACTION;
USE `db_cadcli`;
INSERT INTO `tbuser` (`idUser`, `desLogin`, `desPass`, `desType`, `dtRegister`, `dtUpdate`) VALUES (1, 'admin', '$2y$2y$10$/A8Sje8PxicWArjKzTxbFuF1OvtQceUcuzabMY.jHK/wdUP3eB6yW', 1, NOW(), NOW());
...
START TRANSACTION;
USE `db_cadcli`;
INSERT INTO `tbperson` (`idPerson`, `idUser`, `desName`, `desEmail`, `dtRegister`, `dtUpdate`) VALUES (1, 1, 'Administrador', 'administrador@cadcli.com.br', NOW(), NOW());
